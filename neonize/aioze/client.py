...
class NewAClient:
    ...
    async def build_poll_vote_creation(
        ...
        if model.Error:
            raise BuildPollVoteCreationError(model.Error)  # To be replaced with a custom exception
    ...
    async def _process_single_pack(
        ...
        funcs = [
            self._upload_sticker(sticker, animated, zip_dict) for sticker, animated in stickers
        ]
